-- Fix get_concept_bundle to traverse BOTH outgoing and incoming edges.
-- Previously only traversed outgoing edges, which missed regulatory_claim -> acs_element
-- applies_in_scenario edges (claims point TO elements, not FROM them).
--
-- Approach: Single recursive UNION ALL that joins on both directions using
-- a lateral union of outgoing + incoming neighbors.
...
CREATE OR REPLACE FUNCTION get_concept_bundle(
  ...
  WITH RECURSIVE graph AS (
    ...
    -- Traverse both outgoing and incoming edges in one step
