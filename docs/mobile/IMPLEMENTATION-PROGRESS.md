...
| **M3 — Voice (examiner TTS playback)** | ✅ **DONE & VERIFIED IN DEV BUILD** | `useExaminerVoice` + Practice integration: examiner SPEAKS each turn via `/api/tts` (Deepgram Aura-2 mp3) → `expo-audio`, with barge-in, a voice toggle, and text-only fallback. **Verified end-to-end in a custom iOS dev build** (Expo Go lacks `expo-audio`): the resumed exam rendered with voice ON and fired `POST /api/tts → 200` from the running app. STT (speaking answers) + hard latency/AEC thresholds remain the **physical-device** wall. |
